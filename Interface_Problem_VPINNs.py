...
# Exact Solution & Data Generation
...
def exact_solution_np(x: np.ndarray) -> np.ndarray:
    ...
    - Continuous at x = 0.5  (both sides = 0.25)
    - Derivative jump [du/dx] = 1 - (-1) = 2  → g_I = 2
    - -Delta(u) = -u_xx = -2 everywhere         → f   = -2
    """
...
def generate_data(N_interior: int = 2000,
                  ...
                  seed:        int = 42) -> dict:
    """
    Generates all collocation point sets for the interface problem.

    Domain  : [0,1] x [0,1]
    Omega_1 : x in [0,   0.5] x [0, 1]
    Omega_2 : x in [0.5, 1.0] x [0, 1]
    Gamma   : x = 0.5,          y in [0, 1]   (interface)
    d_Omega : four edges of the unit square    (Dirichlet boundary)
    """
    ...
    n_each = N_boundary // 4
    b_x0  = np.stack([np.zeros(n_each),                         rng.uniform(0, 1, n_each)], axis=1)  # x=0
    b_x1  = np.stack([np.ones(n_each),                          rng.uniform(0, 1, n_each)], axis=1)  # x=1
    b_y0  = np.stack([rng.uniform(0, 1, n_each),                np.zeros(n_each)         ], axis=1)  # y=0
    b_y1  = np.stack([rng.uniform(0, 1, n_each),                np.ones(n_each)          ], axis=1)  # y=1
    bndry_xy  = np.concatenate([b_x0, b_x1, b_y0, b_y1], axis=0)
    bndry_u   = exact_solution_np(bndry_xy[:, 0])               # u depends only on x

    boundary_pts = torch.tensor(bndry_xy, dtype=torch.float32)
    boundary_u   = torch.tensor(bndry_u,  dtype=torch.float32).unsqueeze(1)  # shape (N, 1)
...
# Both Trigonometric and Legendre (bubble)
# All functions vanish on d_Omega = boundary of [0,1]^2
...
class TestFunctions:
    ...
    def get_test_fn(coords: torch.Tensor,
        ...
        if fn_type == "trig":
            ...
            # Map [0,1] -> [-1,1]: t = 2*coord - 1,  dt/dcoord = 2
            tx, ty = 2*x - 1,  2*y - 1
            ...
            wx,  wy  = x*(1 - x),   y*(1 - y)   # window functions (vanish on edges)
            dwx, dwy = 1 - 2*x,     1 - 2*y     # d(window)/d(coord)

            v   = wx * Pm * wy * Pn
            # chain rule:  d/dx [wx * Pm(2x-1)] = dwx*Pm + wx*dPm*2
...
def compute_variational_loss(model:            nn.Module,
    ...
    if test_modes is None:
        # 4x4 grid of modes — better coverage than 5 hand-picked pairs
        test_modes = [(m, n) for m in range(1, 5) for n in range(1, 5)]   # 16 modes

    if fn_types is None:
        fn_types = ["trig", "legendre"]

    # Compute u and its spatial gradients once; reuse across all test functions
...
def train(epochs:        int   = 5000,
    ...
    # Combine Omega_1 + Omega_2 for full-domain variational integration
    interior_coords  = torch.cat([data["interior_1"], data["interior_2"]], dim=0)
    interior_coords.requires_grad_(True)          # needed for autograd grad(u)

    interface_coords = data["interface_pts"]
    bndry_coords     = data["boundary_pts"]
    bndry_u_true     = data["boundary_u"]         # already shape (N, 1)
    ...
    # Halve LR every 2000 epochs to refine convergence
